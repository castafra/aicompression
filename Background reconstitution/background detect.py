...
data = {'box': [[223,287,707,655],[230,817,530,1749], [120, 1291, 210, 1390]], 'class': ['image', 'text', 'logo']}
...
def fill_black_1(detected_objects, image):
    ...
    for i in range(len(detected_objects)):
        ...
        while x< detected_objects['box'].loc[i][3] and vert_band_not_found:
            ...
            vert_band_not_found=vert_band_not_found and (image.getpixel((x, y)) == image.getpixel((x+1, y)))
            x+=1
        ...
        #add vertical band research
            ...
            while y< detected_objects['box'].loc[i][2] and hor_band_not_found:
                #r, g, b = image.getpixel((x, y))
                hor_band_not_found = hor_band_not_found and (image.getpixel((x, y)) == image.getpixel((x, y+1)))
                y+=1
            if not hor_band_not_found:
                ...
                draw_slide.rectangle([(detected_objects['box'].loc[i][1],y),
                                          (detected_objects['box'].loc[i][3],
                                               detected_objects['box'].loc[i][2])], outline = '#000000',
                                                    fill = image.getpixel((x+1, y+2)) )
                print(x,y, 'horizontal found')
            else: #a check
                draw_slide.rectangle([(detected_objects['box'].loc[i][1],
                                 detected_objects['box'].loc[i][0]),
                                     (x, detected_objects['box'].loc[i][2])],
                                         fill = image.getpixel((x,y)) )
                print(x,y, 'horizontal and vertical not found')
            ...
            draw_slide.rectangle([(detected_objects['box'].loc[i][1],
                                 detected_objects['box'].loc[i][0]),
                                     (x, detected_objects['box'].loc[i][2])],
                                         (r0,g0,b0) )
            print(x,y, 'vertical found')
            draw_slide.rectangle([(x,detected_objects['box'].loc[i][0]),
                                  (detected_objects['box'].loc[i][3], 
                                   detected_objects['box'].loc[i][2])] , image.getpixel((x+1, y)) )
